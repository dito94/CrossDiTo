...
void HomeActivity::loop() {
  ...
  if (isCarousel) {
    ...
    if (mappedInput.wasScreenTouchReleased()) {
      // A swipe or drag-off is not a cover tap. Clear the deferred press so it
      // cannot leak into a later contact.
      carouselCoverTouch.cancel();
    }
...
void HomeActivity::render(RenderLock&&) {
  ...
  const auto displayHomeBuffer = [this] {
    const auto refreshMode = initialFullRefresh    ? HalDisplay::FULL_REFRESH
                             : initialCleanRefresh ? HalDisplay::HALF_REFRESH
                                                   : HalDisplay::FAST_REFRESH;
    initialFullRefresh = false;
    initialCleanRefresh = false;
