...
ReaderMenuTabLayout readerMenuTabLayout(const Rect& safe, const int screenHeight, const MappedInputManager& mappedInput,
                                        const int topPadding, const int headerHeight, const int progressHeight,
                                        const int baseTabBarHeight, const int verticalSpacing) {
  const int height = readerMenuTabBarHeight(baseTabBarHeight, mappedInput.hasTouch());
  const int topTabY = safe.y + topPadding + headerHeight + progressHeight;
  const int bottomInset = screenHeight - (safe.y + safe.height);
...
EpubReaderMenuActivity::EpubReaderMenuActivity(
    ...
    : Activity("EpubReaderMenu", renderer, mappedInput),
      menuItems(buildMenuItems(hasFootnotes, hasBookmarks, hasClippings, isCurrentPageBookmarked, isBookCompleted,
                               showReadingPaceReset, hasDictionary, hasPreviousReadingPosition)),
      title(title),
...
EpubReaderMenuActivity::TabMenuItems EpubReaderMenuActivity::buildMenuItems(
    ...
    bool showReadingPaceReset, bool hasDictionary, bool hasPreviousReadingPosition) {
  TabMenuItems items;
...
bool EpubReaderMenuActivity::activateSelectedItem() {
  ...
  if (selectedAction == MenuAction::READER_OPTIONS) {
    ...
    startActivityForResult(makeUniqueNoThrow<ReaderOptionsActivity>(
  ...
  if (selectedAction == MenuAction::CONTROLS_OPTIONS) {
    startActivityForResult(makeUniqueNoThrow<ControlsOptionsActivity>(renderer, mappedInput),
...
void EpubReaderMenuActivity::buildMenuScreen(UiApp::ScreenType& screen) {
  ...
  const auto tabLayout = readerMenuTabLayout(safe, renderer.getScreenHeight(), mappedInput, metrics.topPadding,
...
void EpubReaderMenuActivity::render(RenderLock&&) {
  ...
  const auto tabLayout = readerMenuTabLayout(screen, renderer.getScreenHeight(), mappedInput, metrics.topPadding,
                                             TouchHeaderBackButton::height(metrics, mappedInput), metrics.tabBarHeight,
                                             metrics.tabBarHeight, metrics.verticalSpacing);
  ...
  const Rect tabRect{screen.x, tabLayout.y, screen.width, tabLayout.height};
  drawIconTabBar(tabRect, !tabLayout.atBottom);
