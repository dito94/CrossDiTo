...
class EpubReaderActivity final : public Activity {
 ...
 private:
  ...
  // Exact laid-out whole-book page numbers are persisted as a small SD index.
  // Keep only the active spine's prefix in RAM; even very large omnibus EPUBs
  // therefore add a fixed amount of reader state rather than one entry per spine.
  enum class WholeBookPaginationBuildState : uint8_t { Idle, ScanSpine, BuildSpine };
  uint32_t wholeBookPageCount = 0;
  uint32_t wholeBookCurrentSpineOffset = 0;
  uint32_t wholeBookPaginationLayoutSignature = 0;
  uint32_t wholeBookPaginationFailedSignature = 0;
  int wholeBookPaginationSpineIndex = -1;
  bool wholeBookPaginationExact = false;
  WholeBookPaginationBuildState wholeBookPaginationBuildState = WholeBookPaginationBuildState::Idle;
  ReaderRenderSpec wholeBookPaginationBuildSpec;
  std::unique_ptr<Section> wholeBookPaginationBuildSection;
  uint32_t wholeBookPaginationBuildSignature = 0;
  uint32_t wholeBookPaginationBuildTotalPages = 0;
  int wholeBookPaginationBuildSpineIndex = 0;
  bool wholeBookPaginationBuildPausedForLowMemory = false;
  bool wholeBookPaginationBuildSuppressedImages = false;
  ...
  const Section* prefetchedPageSection = nullptr;
  int prefetchedPageSpine = -1;
  int prefetchedPageNumber = -1;
  int prefetchedPageFontId = 0;
  uint32_t prefetchedPageLayoutSignature = 0;
  bool pendingRecentBookUpdate = false;
  ...
  void resetSection();
  std::unique_ptr<Page> takePrefetchedPage(int pageNumber, int fontId);
  void commitPendingRecentBookUpdate();
  ...
  void armWholeBookPagination(uint16_t viewportWidth, uint16_t viewportHeight);
  void tickWholeBookPagination();
  bool beginWholeBookPaginationBuild(const ReaderRenderSpec& spec, uint32_t layoutSignature);
  bool appendWholeBookPaginationOffset(uint32_t offset) const;
  bool finishWholeBookPaginationBuild();
  void failWholeBookPaginationBuild(const char* reason);
  void resetWholeBookPaginationBuild(bool removeTempFile);
  void yieldWholeBookPaginationSpineToReader();
  bool loadWholeBookPaginationIndex(uint32_t layoutSignature, int spineIndex);
  void clearWholeBookPaginationState();
  void invalidateWholeBookPaginationIndex();
  ...
  bool jumpToPercent(int percent);
  ...
  [[nodiscard]] bool hasPreviousReadingPosition() const;
  ...
  // Footnote navigation
  bool navigateToHref(const std::string& href, bool savePosition = false, bool preferFootnotePreview = false);
 ...
 public:
  ...
  bool sectionBuildWantsTick() const {
    const bool completingCurrentSpineForWholeBook =
        SETTINGS.indexingMethod == CrossPointSettings::INDEXING_FULL_BOOK &&
        wholeBookPaginationBuildState != WholeBookPaginationBuildState::Idle &&
        wholeBookPaginationBuildSignature == activeSectionLayoutSignature && !activeFootnotePreview;
    return section && section->isBuilding() &&
           (!section->activeBuildHasCaughtReadablePages() ||
            static_cast<int>(section->pageCount) < section->currentPage + BUILD_WINDOW_AHEAD ||
            completingCurrentSpineForWholeBook);
  }
  bool wholeBookPaginationWantsTick() const {
    return wholeBookPaginationBuildState != WholeBookPaginationBuildState::Idle &&
           !wholeBookPaginationBuildPausedForLowMemory;
  }
  bool backgroundSectionBuildHasHeap();
  void idlePrewarmNextPage();
  bool skipLoopDelay() override {
    const bool currentSectionWork = sectionBuildWantsTick() && !backgroundBuildPausedForLowMemory;
    return (currentSectionWork || wholeBookPaginationWantsTick()) &&
           !backgroundBuildYieldForInput.load(std::memory_order_relaxed);
  }
  unsigned long nextLoopWakeDelayMs() const override;
