...
void IntervalSelectionActivity::loop() {
  ...
  if (mappedInput.hasTouch() && mappedInput.wasScreenTouchDown(tx, ty)) {
    if (contains(touchActionButtonRect(touchScreen, false), tx, ty)) {
      // The activity closes on touch-down; suppress the matching release so it
      // cannot activate the resumed settings screen.
...
void IntervalSelectionActivity::render(RenderLock&&) {
  ...
  } else if (showClockTimeValue) {
    FrontlightSchedule::formatTimeSlot(static_cast<uint8_t>(value), SETTINGS.clockFormat == 1, formattedValue,
