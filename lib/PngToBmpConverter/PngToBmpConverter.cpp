...
bool PngToBmpConverter::pngFileToBmpStreamInternal(FsFile& pngFile, Print& bmpOut, int targetWidth, int targetHeight,
  ...
  if (oneBit) {
    ...
    if (!atkinson1BitDitherer || !atkinson1BitDitherer->ready()) {
    ...
    if (USE_ATKINSON) {
      ...
      if (!atkinsonDitherer || !atkinsonDitherer->ready()) {
      ...
      if (!fsDitherer || !fsDitherer->ready()) {
  ...
  std::unique_ptr<uint32_t[]> scalingWorkspace;
  int currentOutY = 0;
  uint32_t nextOutY_srcStart = 0;

  if (needsScaling) {
    const size_t rowCountWords =
        (static_cast<size_t>(outWidth) * sizeof(uint16_t) + sizeof(uint32_t) - 1) / sizeof(uint32_t);
    scalingWorkspace = makeUniqueNoThrow<uint32_t[]>(static_cast<size_t>(outWidth) + rowCountWords);
    if (!scalingWorkspace) {
      LOG_ERR("PNG", "OOM: scaling workspace for %d-pixel row", outWidth);
      free(rowBuffer);
      free(ctx.currentRow);
      free(ctx.previousRow);
      return false;
    }
    rowAccum = scalingWorkspace.get();
    rowCount = reinterpret_cast<uint16_t*>(rowAccum + outWidth);
