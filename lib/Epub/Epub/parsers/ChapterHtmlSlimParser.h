...
class ChapterHtmlSlimParser {
 ...
 private:
  ...
  // Layout-neutral Expat event cache. The first successful parse writes a
  // compact sequential stream; later layout changes replay it through the same
  // semantic callbacks without inflating or tokenizing XHTML again.
  static constexpr uint8_t MAX_COMPILED_ATTRIBUTES = 32;
  static constexpr size_t COMPILED_MAX_PAYLOAD_SIZE = 4096;
  static constexpr size_t COMPILED_RECORD_HEADER_SIZE = sizeof(uint8_t) + sizeof(uint16_t);
  static constexpr size_t COMPILED_IO_BUFFER_SIZE = COMPILED_RECORD_HEADER_SIZE + COMPILED_MAX_PAYLOAD_SIZE;
  std::string compiledCachePath_;
  std::string compiledCacheTmpPath_;
  FsFile compiledOutputFile_;
  std::unique_ptr<uint8_t[]> compiledRecordBuffer_;
  std::array<const char*, MAX_COMPILED_ATTRIBUTES * 2 + 1> compiledAttributePointers_{};
  bool compiledReplayActive_ = false;
  bool compiledWriteFailed_ = false;
  bool compiledWriteActive_ = false;
  uint32_t sourceHtmlSize_ = 0;
  size_t compiledOutputOffset_ = 0;
  ...
  bool ensureCompiledOutputOpen();
  bool beginCompiledReplay();
  bool beginCompiledWrite();
  bool validateCompiledStream(size_t cacheSize);
  bool processCompiledRecord(uint8_t type, uint16_t payloadLength, bool dispatch);
  ParseStatus replayCompiledStep();
  bool writeCompiledRecord(uint8_t type, const void* payload, uint16_t payloadLength);
  bool writeCompiledStartElement(const XML_Char* name, const XML_Char** atts);
  bool writeCompiledData(uint8_t type, const XML_Char* data, int len);
  bool writeCompiledEndElement(const XML_Char* name);
  void failCompiledWrite(const char* reason);
  bool commitCompiledWrite();
  void closeCompiledFiles(bool removeTemp);
  void prewarmCompiledAdvanceTable();
  static void XMLCALL recordingStartElement(void* userData, const XML_Char* name, const XML_Char** atts);
  static void XMLCALL recordingCharacterData(void* userData, const XML_Char* s, int len);
  static void XMLCALL recordingDefaultHandlerExpand(void* userData, const XML_Char* s, int len);
  static void XMLCALL recordingEndElement(void* userData, const XML_Char* name);
 ...
 public:
  explicit ChapterHtmlSlimParser(
      ...
      const uint16_t previewMaxPages = 0, std::string compiledCachePath = {})
      ...
      : epub(&epub),
        ...
        compiledCachePath_(std::move(compiledCachePath)),
        compiledCacheTmpPath_(compiledCachePath_.empty() ? std::string{} : compiledCachePath_ + ".part"),
