...
// Layout-neutral XHTML event stream. Files are atomically promoted only after
// a complete, well-formed parse, so the end marker doubles as a cheap torn-file
// check before replay.
constexpr uint32_t COMPILED_CACHE_MAGIC = 0x545843FF;  // bytes: 0xFF, "CXT"
constexpr uint8_t COMPILED_CACHE_VERSION = 1;
constexpr size_t COMPILED_CACHE_HEADER_SIZE =
    sizeof(COMPILED_CACHE_MAGIC) + sizeof(COMPILED_CACHE_VERSION) + sizeof(uint32_t);
enum class CompiledEvent : uint8_t {
  StartElement = 1,
  CharacterData = 2,
  DefaultData = 3,
  EndElement = 4,
...
bool readExact(FsFile& file, void* data, const size_t size) {
  return size == 0 || file.read(static_cast<uint8_t*>(data), size) == size;
}

bool writeExact(FsFile& file, const void* data, const size_t size) {
  return size == 0 || file.write(static_cast<const uint8_t*>(data), size) == size;
}

bool readCompiledRecordHeader(FsFile& file, uint8_t& type, uint16_t& payloadLength) {
  uint8_t header[sizeof(type) + sizeof(payloadLength)] = {};
  if (!readExact(file, header, sizeof(header))) return false;
  type = header[0];
  memcpy(&payloadLength, header + sizeof(type), sizeof(payloadLength));
  return true;
}
...
bool ChapterHtmlSlimParser::beginCompiledReplay() {
  if (isPreviewBuild() || compiledCachePath_.empty() || !Storage.exists(compiledCachePath_.c_str())) {
    return false;
  }

  FsFile source;
  if (!Storage.openFileForRead("EHP", filepath, source)) return false;
  const size_t sourceSize = source.size();
  source.close();
  if (sourceSize > UINT32_MAX) {
    LOG_DBG("EHP", "Compiled cache disabled for oversized spine");
    return false;
  }
  sourceHtmlSize_ = static_cast<uint32_t>(sourceSize);

  if (!Storage.openFileForRead("EHP", compiledCachePath_, parseFile_)) return false;
  const size_t cacheSize = parseFile_.size();
  uint8_t header[COMPILED_CACHE_HEADER_SIZE] = {};
  uint32_t magic = 0;
  uint32_t cachedSourceSize = 0;
  bool valid = cacheSize >= COMPILED_CACHE_HEADER_SIZE + COMPILED_RECORD_HEADER_SIZE &&
               readExact(parseFile_, header, sizeof(header));
  if (valid) {
    memcpy(&magic, header, sizeof(magic));
    memcpy(&cachedSourceSize, header + sizeof(magic) + sizeof(COMPILED_CACHE_VERSION), sizeof(cachedSourceSize));
    valid = magic == COMPILED_CACHE_MAGIC && header[sizeof(magic)] == COMPILED_CACHE_VERSION &&
            cachedSourceSize == sourceHtmlSize_;
  }

  // One bounded heap buffer is reused for validation, replay, and cache writes.
  // It is too large for the reader task stack and avoids per-event allocation churn.
  if (valid) {
    compiledRecordBuffer_ = makeUniqueNoThrow<uint8_t[]>(COMPILED_IO_BUFFER_SIZE);
    if (!compiledRecordBuffer_) {
      LOG_ERR("EHP", "OOM: compiled spine I/O buffer (%u bytes)", static_cast<unsigned>(COMPILED_IO_BUFFER_SIZE));
      parseFile_.close();
      return false;
    }
    valid = validateCompiledStream(cacheSize);
  }

  if (!valid) {
    parseFile_.close();
    compiledRecordBuffer_.reset();
    Storage.remove(compiledCachePath_.c_str());
    LOG_DBG("EHP", "Discarded invalid compiled spine cache");
    return false;
  }

  if (!parseFile_.seek(COMPILED_CACHE_HEADER_SIZE)) {
    parseFile_.close();
    compiledRecordBuffer_.reset();
    return false;
  }
  parseFileOffset_ = COMPILED_CACHE_HEADER_SIZE;
  parseFileSize_ = cacheSize;
  compiledReplayActive_ = true;
  LOG_DBG("EHP", "Using compiled spine cache: %s (%u bytes)", compiledCachePath_.c_str(),
          static_cast<unsigned>(cacheSize));
  return true;
}

bool ChapterHtmlSlimParser::validateCompiledStream(const size_t cacheSize) {
  if (!parseFile_ || !compiledRecordBuffer_ || !parseFile_.seek(COMPILED_CACHE_HEADER_SIZE)) return false;

  while (parseFile_.position() < cacheSize) {
    const size_t remaining = cacheSize - parseFile_.position();
    uint8_t type = 0;
    uint16_t payloadLength = 0;
    if (remaining < COMPILED_RECORD_HEADER_SIZE || !readCompiledRecordHeader(parseFile_, type, payloadLength) ||
        payloadLength > COMPILED_MAX_PAYLOAD_SIZE || payloadLength > cacheSize - parseFile_.position() ||
        !readExact(parseFile_, compiledRecordBuffer_.get(), payloadLength)) {
      return false;
    }
    if (type == static_cast<uint8_t>(CompiledEvent::EndOfStream)) {
      return payloadLength == 0 && parseFile_.position() == cacheSize;
    }
    if (!processCompiledRecord(type, payloadLength, false)) return false;
  }
  return false;
}

bool ChapterHtmlSlimParser::processCompiledRecord(const uint8_t type, const uint16_t payloadLength,
                                                  const bool dispatch) {
  auto nextString = [&](size_t& offset) -> const char* {
    if (offset >= payloadLength) return nullptr;
    char* const start = reinterpret_cast<char*>(compiledRecordBuffer_.get() + offset);
    const void* const end = memchr(start, '\0', payloadLength - offset);
    if (!end) return nullptr;
    offset = static_cast<const char*>(end) - reinterpret_cast<const char*>(compiledRecordBuffer_.get()) + 1;
    return start;
  };

  if (type == static_cast<uint8_t>(CompiledEvent::StartElement)) {
    if (payloadLength < 2) return false;
    const uint8_t attributeCount = compiledRecordBuffer_[0];
    if (attributeCount > MAX_COMPILED_ATTRIBUTES) return false;
    size_t offset = 1;
    const char* const name = nextString(offset);
    if (!name) return false;
    for (uint8_t i = 0; i < attributeCount * 2; ++i) {
      compiledAttributePointers_[i] = nextString(offset);
      if (!compiledAttributePointers_[i]) return false;
    }
    compiledAttributePointers_[attributeCount * 2] = nullptr;
    if (offset != payloadLength) return false;
    if (dispatch) startElement(this, name, compiledAttributePointers_.data());
  } else if (type == static_cast<uint8_t>(CompiledEvent::EndElement)) {
    size_t offset = 0;
    const char* const name = nextString(offset);
    if (!name || offset != payloadLength) return false;
    if (dispatch) endElement(this, name);
  } else if (type == static_cast<uint8_t>(CompiledEvent::CharacterData)) {
    if (dispatch) characterData(this, reinterpret_cast<const char*>(compiledRecordBuffer_.get()), payloadLength);
  } else if (type == static_cast<uint8_t>(CompiledEvent::DefaultData)) {
    if (dispatch) defaultHandlerExpand(this, reinterpret_cast<const char*>(compiledRecordBuffer_.get()), payloadLength);
  } else {
    return false;
  }
  return !dispatch || !lowMemoryAbort;
}

bool ChapterHtmlSlimParser::ensureCompiledOutputOpen() {
  if (!compiledWriteActive_) return false;
  if (compiledOutputFile_) return true;
  if (compiledOutputOffset_ < COMPILED_CACHE_HEADER_SIZE) {
    failCompiledWrite("invalid resume offset");
    return false;
  }

  compiledOutputFile_ = Storage.open(compiledCacheTmpPath_.c_str(), O_RDWR);
  if (!compiledOutputFile_ || compiledOutputFile_.size() != compiledOutputOffset_ ||
      !compiledOutputFile_.seek(compiledOutputOffset_)) {
    failCompiledWrite("cache writer reopen");
    return false;
  }
  return true;
}

bool ChapterHtmlSlimParser::beginCompiledWrite() {
  if (isPreviewBuild() || compiledCachePath_.empty() || Storage.exists(compiledCachePath_.c_str()) ||
      sourceHtmlSize_ == 0) {
    return false;
  }
  compiledWriteFailed_ = false;
  compiledRecordBuffer_ = makeUniqueNoThrow<uint8_t[]>(COMPILED_IO_BUFFER_SIZE);
  if (!compiledRecordBuffer_) {
    LOG_ERR("EHP", "OOM: compiled spine I/O buffer (%u bytes)", static_cast<unsigned>(COMPILED_IO_BUFFER_SIZE));
    return false;
  }
  if (Storage.exists(compiledCacheTmpPath_.c_str())) Storage.remove(compiledCacheTmpPath_.c_str());
  if (!Storage.openFileForWrite("EHP", compiledCacheTmpPath_, compiledOutputFile_)) {
    compiledRecordBuffer_.reset();
    return false;
  }

  uint8_t header[COMPILED_CACHE_HEADER_SIZE] = {};
  memcpy(header, &COMPILED_CACHE_MAGIC, sizeof(COMPILED_CACHE_MAGIC));
  header[sizeof(COMPILED_CACHE_MAGIC)] = COMPILED_CACHE_VERSION;
  memcpy(header + sizeof(COMPILED_CACHE_MAGIC) + sizeof(COMPILED_CACHE_VERSION), &sourceHtmlSize_,
         sizeof(sourceHtmlSize_));
  if (!writeExact(compiledOutputFile_, header, sizeof(header))) {
    failCompiledWrite("header write");
    return false;
  }
  compiledOutputOffset_ = compiledOutputFile_.position();
  compiledWriteActive_ = true;
  return true;
}

void ChapterHtmlSlimParser::failCompiledWrite(const char* reason) {
  if (compiledWriteFailed_) return;
  compiledWriteFailed_ = true;
  compiledWriteActive_ = false;
  if (compiledOutputFile_) compiledOutputFile_.close();
  compiledOutputOffset_ = 0;
  if (!compiledCacheTmpPath_.empty() && Storage.exists(compiledCacheTmpPath_.c_str())) {
    Storage.remove(compiledCacheTmpPath_.c_str());
  }
  if (!compiledReplayActive_) compiledRecordBuffer_.reset();
  LOG_ERR("EHP", "Compiled spine cache disabled: %s", reason);
}

bool ChapterHtmlSlimParser::writeCompiledRecord(const uint8_t type, const void* payload, const uint16_t payloadLength) {
  if (!compiledWriteActive_ || !compiledRecordBuffer_ || payloadLength > COMPILED_MAX_PAYLOAD_SIZE ||
      (payloadLength > 0 && !payload) || !ensureCompiledOutputOpen()) {
    return false;
  }

  compiledRecordBuffer_[0] = type;
  memcpy(compiledRecordBuffer_.get() + sizeof(type), &payloadLength, sizeof(payloadLength));
  if (payloadLength > 0) {
    memcpy(compiledRecordBuffer_.get() + COMPILED_RECORD_HEADER_SIZE, payload, payloadLength);
  }
  if (!writeExact(compiledOutputFile_, compiledRecordBuffer_.get(), COMPILED_RECORD_HEADER_SIZE + payloadLength)) {
    failCompiledWrite("record write");
    return false;
  }
  compiledOutputOffset_ = compiledOutputFile_.position();
  return true;
...
bool ChapterHtmlSlimParser::writeCompiledStartElement(const XML_Char* name, const XML_Char** atts) {
  if (!compiledWriteActive_) return false;
  uint8_t attributeCount = 0;
  if (atts) {
    while (atts[attributeCount * 2] != nullptr) {
      if (attributeCount >= MAX_COMPILED_ATTRIBUTES || atts[attributeCount * 2 + 1] == nullptr) {
        failCompiledWrite("too many or malformed attributes");
        return false;
      }
      ++attributeCount;
    }
  ...
  size_t payloadSize = sizeof(attributeCount) + strlen(name) + 1;
  for (uint8_t i = 0; i < attributeCount; ++i) {
    payloadSize += strlen(atts[i * 2]) + 1;
    payloadSize += strlen(atts[i * 2 + 1]) + 1;
  }
  if (payloadSize > COMPILED_MAX_PAYLOAD_SIZE || payloadSize > UINT16_MAX) {
    failCompiledWrite("start-element record too large");
    return false;
  }

  uint8_t* const payload = compiledRecordBuffer_.get() + COMPILED_RECORD_HEADER_SIZE;
  size_t offset = 0;
  payload[offset++] = attributeCount;
  const size_t nameLength = strlen(name) + 1;
  memcpy(payload + offset, name, nameLength);
  offset += nameLength;
  for (uint8_t i = 0; i < attributeCount; ++i) {
    const size_t keyLength = strlen(atts[i * 2]) + 1;
    const size_t valueLength = strlen(atts[i * 2 + 1]) + 1;
    memcpy(payload + offset, atts[i * 2], keyLength);
    offset += keyLength;
    memcpy(payload + offset, atts[i * 2 + 1], valueLength);
    offset += valueLength;
  }
  return offset == payloadSize && writeCompiledRecord(static_cast<uint8_t>(CompiledEvent::StartElement), payload,
                                                      static_cast<uint16_t>(payloadSize));
}

bool ChapterHtmlSlimParser::writeCompiledData(const uint8_t type, const XML_Char* data, const int len) {
  if (!compiledWriteActive_) return false;
  if (len < 0 || static_cast<size_t>(len) > COMPILED_MAX_PAYLOAD_SIZE || len > UINT16_MAX) {
    failCompiledWrite("text record too large");
    return false;
  }
  return writeCompiledRecord(type, data, static_cast<uint16_t>(len));
}

bool ChapterHtmlSlimParser::writeCompiledEndElement(const XML_Char* name) {
  if (!compiledWriteActive_) return false;
  const size_t payloadSize = strlen(name) + 1;
  if (payloadSize > COMPILED_MAX_PAYLOAD_SIZE || payloadSize > UINT16_MAX) {
    failCompiledWrite("end-element record too large");
    return false;
  }
  return writeCompiledRecord(static_cast<uint8_t>(CompiledEvent::EndElement), name, static_cast<uint16_t>(payloadSize));
}

bool ChapterHtmlSlimParser::commitCompiledWrite() {
  if (!compiledWriteActive_ || compiledWriteFailed_ || malformedMarkupTruncated) {
    closeCompiledFiles(true);
    return false;
  }
  const bool written =
      writeCompiledRecord(static_cast<uint8_t>(CompiledEvent::EndOfStream), nullptr, 0) && compiledOutputFile_.sync();
  compiledOutputFile_.close();
  compiledWriteActive_ = false;
  compiledOutputOffset_ = 0;
  if (!written || !Storage.rename(compiledCacheTmpPath_.c_str(), compiledCachePath_.c_str())) {
    Storage.remove(compiledCacheTmpPath_.c_str());
    LOG_ERR("EHP", "Failed to commit compiled spine cache");
    return false;
  }
  LOG_DBG("EHP", "Committed compiled spine cache: %s", compiledCachePath_.c_str());
  return true;
}

void ChapterHtmlSlimParser::closeCompiledFiles(const bool removeTemp) {
  if (compiledOutputFile_) compiledOutputFile_.close();
  compiledWriteActive_ = false;
  compiledOutputOffset_ = 0;
  if (removeTemp && !compiledCacheTmpPath_.empty() && Storage.exists(compiledCacheTmpPath_.c_str())) {
    Storage.remove(compiledCacheTmpPath_.c_str());
  }
}

void XMLCALL ChapterHtmlSlimParser::recordingStartElement(void* userData, const XML_Char* name, const XML_Char** atts) {
  auto* self = static_cast<ChapterHtmlSlimParser*>(userData);
  self->writeCompiledStartElement(name, atts);
  startElement(userData, name, atts);
}

void XMLCALL ChapterHtmlSlimParser::recordingCharacterData(void* userData, const XML_Char* s, const int len) {
  auto* self = static_cast<ChapterHtmlSlimParser*>(userData);
  self->writeCompiledData(static_cast<uint8_t>(CompiledEvent::CharacterData), s, len);
  characterData(userData, s, len);
}

void XMLCALL ChapterHtmlSlimParser::recordingDefaultHandlerExpand(void* userData, const XML_Char* s, const int len) {
  auto* self = static_cast<ChapterHtmlSlimParser*>(userData);
  self->writeCompiledData(static_cast<uint8_t>(CompiledEvent::DefaultData), s, len);
  defaultHandlerExpand(userData, s, len);
}

void XMLCALL ChapterHtmlSlimParser::recordingEndElement(void* userData, const XML_Char* name) {
  auto* self = static_cast<ChapterHtmlSlimParser*>(userData);
  self->writeCompiledEndElement(name);
  endElement(userData, name);
}

ChapterHtmlSlimParser::ParseStatus ChapterHtmlSlimParser::replayCompiledStep() {
  if (!compiledReplayActive_ || !compiledRecordBuffer_) return ParseStatus::Error;
  if (!ensureInputFileOpen()) return ParseStatus::Error;

  const size_t stepStart = parseFile_.position();
  while (parseFile_.position() - stepStart < PARSE_BUFFER_SIZE) {
    uint8_t type = 0;
    uint16_t payloadLength = 0;
    if (!readCompiledRecordHeader(parseFile_, type, payloadLength) || payloadLength > COMPILED_MAX_PAYLOAD_SIZE ||
        !readExact(parseFile_, compiledRecordBuffer_.get(), payloadLength)) {
      LOG_ERR("EHP", "Corrupt compiled spine cache; rebuilding on next open");
      parseFile_.close();
      Storage.remove(compiledCachePath_.c_str());
      compiledReplayActive_ = false;
      return ParseStatus::Error;
    }
    parseFileOffset_ = parseFile_.position();

    if (type == static_cast<uint8_t>(CompiledEvent::EndOfStream)) {
      if (payloadLength != 0 || parseFile_.position() != parseFile_.size()) {
        LOG_ERR("EHP", "Invalid compiled spine end marker");
        parseFile_.close();
        Storage.remove(compiledCachePath_.c_str());
        compiledReplayActive_ = false;
        return ParseStatus::Error;
    ...
    if (!processCompiledRecord(type, payloadLength, true)) {
      if (!lowMemoryAbort) {
        LOG_ERR("EHP", "Malformed compiled spine record; rebuilding on next open");
        parseFile_.close();
        Storage.remove(compiledCachePath_.c_str());
        compiledReplayActive_ = false;
      }
...
void ChapterHtmlSlimParser::prewarmCompiledAdvanceTable() {
  if (!renderer.isSdCardFont(fontId) || !compiledReplayActive_ || !parseFile_ || !compiledRecordBuffer_) return;
  const auto heap = MemoryBudget::snapshot();
  if (!MemoryBudget::hasHeap(heap, MIN_FREE_HEAP_FOR_SECTION_PREWARM, MIN_MAX_ALLOC_FOR_SECTION_PREWARM)) return;

  auto codepoints = makeUniqueNoThrow<uint32_t[]>(SECTION_ADVANCE_PREWARM_MAX_CODEPOINTS);
  ...
  if (!parseFile_.seek(COMPILED_CACHE_HEADER_SIZE)) return;
  while (!hitCap && parseFile_.available() >= static_cast<int>(COMPILED_RECORD_HEADER_SIZE)) {
    uint8_t type = 0;
    uint16_t payloadLength = 0;
    if (!readCompiledRecordHeader(parseFile_, type, payloadLength) || payloadLength > COMPILED_MAX_PAYLOAD_SIZE ||
        !readExact(parseFile_, compiledRecordBuffer_.get(), payloadLength)) {
    ...
    if (type == static_cast<uint8_t>(CompiledEvent::EndOfStream)) break;
    if (type != static_cast<uint8_t>(CompiledEvent::CharacterData)) continue;
    for (uint16_t i = 0; i < payloadLength && !hitCap; ++i) {
      const uint8_t byte = compiledRecordBuffer_[i];
      if (isWhitespace(static_cast<char>(byte))) {
        resetPrewarmUtf8(utf8Accumulator, utf8Remaining);
        hitCap = appendUniquePrewarmCodepoint(' ', codepoints.get(), cpCount, SECTION_ADVANCE_PREWARM_MAX_CODEPOINTS);
      } else {
        hitCap = feedPrewarmUtf8Byte(byte, codepoints.get(), cpCount, utf8Accumulator, utf8Remaining);
  ...
  parseFile_.seek(COMPILED_CACHE_HEADER_SIZE);
  parseFileOffset_ = COMPILED_CACHE_HEADER_SIZE;
  if (cpCount > 0) {
    renderer.ensureSdCardFontReady(fontId, codepoints.get(), cpCount, true, hyphenationEnabled, 0x0F);
  }
}
...
bool ChapterHtmlSlimParser::ensureInputFileOpen() {
  ...
  const std::string& inputPath = compiledReplayActive_ ? compiledCachePath_ : filepath;
  if (!Storage.openFileForRead("EHP", inputPath, parseFile_)) {
    return false;
  }
...
void ChapterHtmlSlimParser::releaseInputFile() {
  ...
  if (compiledWriteActive_ && compiledOutputFile_) {
    compiledOutputOffset_ = compiledOutputFile_.position();
    const bool synced = compiledOutputFile_.sync();
    const bool closed = compiledOutputFile_.close();
    if (!synced || !closed) failCompiledWrite("cache writer release");
  }
}
...
bool ChapterHtmlSlimParser::beginParse() {
  ...
  // A validated event stream does not need an Expat instance. It still runs
  // through the same semantic callbacks, so layout and rendering stay identical.
  if (beginCompiledReplay()) {
    if (popupFn && sourceHtmlSize_ >= MIN_SIZE_FOR_POPUP) popupFn();
    parseStartTime_ = millis();
    prewarmCompiledAdvanceTable();
    return true;
  }
  ...
  parseFileSize_ = parseFile_.size();
  if (parseFileSize_ <= UINT32_MAX) {
    sourceHtmlSize_ = static_cast<uint32_t>(parseFileSize_);
    beginCompiledWrite();
  } else {
    LOG_DBG("EHP", "Compiled cache disabled for oversized spine");
  }
  ...
  if (compiledWriteActive_) {
    XML_SetDefaultHandlerExpand(activeParser, recordingDefaultHandlerExpand);
    XML_SetElementHandler(activeParser, recordingStartElement, recordingEndElement);
    XML_SetCharacterDataHandler(activeParser, recordingCharacterData);
  } else {
    // Handle HTML entities (like &nbsp;) that aren't in XML spec or DTD.
    // DefaultHandlerExpand preserves normal entity expansion from DOCTYPE.
...
ChapterHtmlSlimParser::ParseStatus ChapterHtmlSlimParser::parseStep() {
  if (compiledReplayActive_) return replayCompiledStep();
  if (!activeParser) {
...
bool ChapterHtmlSlimParser::finishParse() {
  const bool replayedCompiledCache = compiledReplayActive_;
  LOG_DBG("EHP", "Time to parse and build pages (%s): %lu ms (free=%u, maxAlloc=%u)",
          replayedCompiledCache ? "compiled" : "xhtml", millis() - parseStartTime_, ESP.getFreeHeap(),
          ESP.getMaxAllocHeap());
  if (activeParser) {
    destroyXmlParser(activeParser);
    activeParser = nullptr;
  ...
  if (compiledWriteActive_) {
    // Cache creation is optional: a failed promotion must never fail a valid
    // section build or hide readable pages.
    (void)commitCompiledWrite();
  } else {
    closeCompiledFiles(true);
  }
