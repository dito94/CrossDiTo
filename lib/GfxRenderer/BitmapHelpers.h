...
class Atkinson1BitDitherer {
 public:
  explicit Atkinson1BitDitherer(int width) : width(width) {
    ...
    errorRows = makeUniqueNoThrow<int16_t[]>(stride * 3);
    if (!errorRows) return;
    errorRow0 = errorRows.get();
    errorRow1 = errorRow0 + stride;
    errorRow2 = errorRow1 + stride;
  }
...
class FloydSteinbergDitherer {
 public:
  explicit FloydSteinbergDitherer(int width) : width(width), rowCount(0) {
    ...
    errorRows = makeUniqueNoThrow<int16_t[]>(stride * 2);
    if (!errorRows) return;
    errorCurRow = errorRows.get();
    errorNextRow = errorCurRow + stride;
  }
