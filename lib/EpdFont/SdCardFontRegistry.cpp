...
namespace {
bool fileAllocationFailed(const HalFile& file) {
#ifdef SIMULATOR
  // The host-backed HalFile has no fallible wrapper allocation; failed opens
  // are ordinary missing-path errors in the simulator.
  (void)file;
  return false;
#else
...
bool SdCardFontRegistry::scanDirectory(const char* dirPath, SdCardFontFamilyInfo& family) {
  ...
  if (!dir || !dir.isDirectory()) {
    if (fileAllocationFailed(dir)) LOG_ERR("SDREG", "Out of memory opening font directory: %s", dirPath);
    return !fileAllocationFailed(dir);
  }
