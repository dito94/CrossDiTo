...
CrossDiTo 1.5.1 is a personal Xteink X4 Pro test build made directly from CrossInk 1.5.0. Full credit for the base firmware and all inherited functionality belongs to the CrossInk, CrossPoint, and upstream contributors. The notes below list only changes made on top of CrossInk.
...
For the base firmware and its release history, see [CrossInk](https://github.com/uxjulia/CrossInk) and [CrossPoint Reader](https://github.com/crosspoint-reader/crosspoint-reader). CrossDiTo's [changelog](../../CHANGELOG.md) contains only changes made by this fork.
